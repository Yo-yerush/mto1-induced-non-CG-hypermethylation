        ...
        tryCatch(
            {
                ...
                if (context == "CG") {
                    ...
                    yaxis_txt_var <- element_text(size = 12, color = "#4b4b4b")
                } else {
                ...
                xaxis_txt_var <- element_text(size = 12, color = "#4b4b4b")
                ...
                p_points <- p + geom_point(alpha = 0.25, size = 0.5, show.legend = FALSE) +
                    stat_smooth(method = "glm.nb", formula = y ~ x, linewidth = 0.85) +
                    scale_y_continuous(
...
tiff(paste0(path_2_save.2, "lm.stats.plot.", var2_name, ".tif"), width = 5, height = 2, units = "in", res = 600, family = "serif") # width = 4
